...
b}});return a}return c()});
define("SceneRenderer",["UniversalUniverse","OrbitalSolver-Circular"],function(b){return{InitializeRenderer:function(){var c=oCanvas.create({canvas:"#UUCanvasUniverse",background:"#000"}),a=c.width/2,d=c.height/2;b.AddSun();b.Suns[0].Radius=30;b.Suns[0].OrbitalSolver.OrbitalCenterX=a;b.Suns[0].OrbitalSolver.OrbitalCenterY=d;var f=b.Suns[0].OrbitalSolver.ComputeLocation(b.Time);b.Suns[0].Renderable=c.display.ellipse({x:f.X,y:f.Y,radius:b.Suns[0].Radius,fill:"#f00"}).add();for(var f=Math.floor(8*Math.random()+
